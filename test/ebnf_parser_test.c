...
void ebnf_start_parsing__identifier(){
	...
	fsm_thread_fake_start(&thread, fsm_get_state(&fix.fsm, nzs("syntactic_primary")));
...
void ebnf_start_parsing__concatenate(){
	...
	FsmThread thread;
	fsm_thread_init(&thread, &fix.fsm, (Listener) { .function = NULL });
	fsm_thread_fake_start(&thread, fsm_get_state(&fix.fsm, nzs("single_definition")));

	MATCH_SHIFT(thread, fix.META_IDENTIFIER);
	MATCH_REDUCE(thread, fix.CONCATENATE_SYMBOL, fix.SYNTACTIC_PRIMARY);
	MATCH_POP_SHIFT(thread, fix.CONCATENATE_SYMBOL);
	...
	MATCH_POP(thread, fix.CONCATENATE_SYMBOL);
...
void ebnf_start_parsing__separator(){
	...
	FsmThread thread;
	fsm_thread_init(&thread, &fix.fsm, (Listener) { .function = NULL });
	fsm_thread_fake_start(&thread, fsm_get_state(&fix.fsm, nzs("definitions_list")));

	MATCH_SHIFT(thread, fix.META_IDENTIFIER);
	MATCH_REDUCE(thread, fix.DEFINITION_SEPARATOR_SYMBOL, fix.SYNTACTIC_PRIMARY);
	MATCH_POP_SHIFT(thread, fix.DEFINITION_SEPARATOR_SYMBOL);
	...
	MATCH_POP(thread, fix.DEFINITION_SEPARATOR_SYMBOL);
...
void ebnf_start_parsing__syntactic_term(){
	...
	FsmThread thread;
	fsm_thread_init(&thread, &fix.fsm, (Listener) { .function = NULL });
	fsm_thread_fake_start(&thread, fsm_get_state(&fix.fsm, nzs("syntactic_term")));

	MATCH_SHIFT(thread, fix.TERMINAL_STRING);

	MATCH_REDUCE(thread, fix.EXCEPT_SYMBOL, fix.SYNTACTIC_PRIMARY);
	MATCH_POP_SHIFT(thread, fix.EXCEPT_SYMBOL);
	MATCH_REDUCE(thread, fix.EXCEPT_SYMBOL, fix.SYNTACTIC_FACTOR);
	MATCH_POP(thread, fix.EXCEPT_SYMBOL);
...
void ebnf_start_parsing__syntax_rule(){
	...
	FsmThread thread;
	fsm_thread_init(&thread, &fix.fsm, (Listener) { .function = NULL });
	fsm_thread_fake_start(&thread, fsm_get_state(&fix.fsm, nzs("syntax_rule")));
...
void ebnf_start_parsing__group(){
	...
	MATCH_POP_SHIFT(thread, fix.END_GROUP_SYMBOL);
	...
	MATCH_POP(thread, fix.END_GROUP_SYMBOL);
...
void ebnf_start_parsing__syntax(){
	...
	MATCH_POP_SHIFT(thread, fix.SYNTAX_RULE);
	...
	MATCH_REDUCE(thread, L_EOF, fix.SYNTAX_RULE);
	MATCH_POP(thread, fix.SYNTAX_RULE);
