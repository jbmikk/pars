...
#define TEST_ACTION(S, Y, A) \
	...
	t_assert(tran.action->reduction == (A).reduction);
...
void fsm_thread_match__ref_range_collision(){
	FsmBuilder builder;
	Transition tran;
	...
	action3 = *builder.action;

	fsm_builder_terminal(&builder, 'c');
	fsm_builder_end(&builder);

	fsm_builder_define(&builder, nzs("C"));
	fsm_builder_terminal_range(&builder, (Range){'1', '9'});
	action2 = *builder.action;
	...
	fsm_builder_nonterminal(&builder,  nzs("B"));
	fsm_builder_or(&builder);
	fsm_builder_nonterminal(&builder,  nzs("C"));
	fsm_builder_group_end(&builder);
	fsm_builder_end(&builder);
	...
	TEST_ACTION(thread, '2', action2);
	TEST_ACTION(thread, '3', action2);
	TEST_ACTION(thread, '4', action1);
	TEST_ACTION(thread, '5', action2);
	TEST_ACTION(thread, '6', action3);
	TEST_ACTION(thread, '7', action2);
	TEST_ERROR(thread, 'a');
...
int main(int argc, char** argv){
	...
	*/
	t_test(fsm_thread_match__ref_range_collision);
