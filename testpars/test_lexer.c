...
void setup(Fixture *fix, gconstpointer data){
	fix->input_integer = input_init_buffer(I_INTEGER, strlen(I_INTEGER));
	fix->input_identifier = input_init_buffer(I_IDENTIFIER, strlen(I_IDENTIFIER));
	fix->input_terminal_string = input_init_buffer(I_TERMINAL_STRING, strlen(I_TERMINAL_STRING));
	fix->input_rule_one = input_init_buffer(I_RULE_ONE, strlen(I_RULE_ONE));
}

void teardown(Fixture *fix, gconstpointer data){
	input_close(fix->input_integer);
	input_close(fix->input_identifier);
	input_close(fix->input_rule_one);
}
